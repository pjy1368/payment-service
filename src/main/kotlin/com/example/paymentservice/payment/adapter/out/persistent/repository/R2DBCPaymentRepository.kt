...
class R2DBCPaymentRepository(
    ...
    override suspend fun save(paymentEvent: PaymentEvent) {
        transactionalOperator.executeAndAwait {
            val paymentEventId = insertPaymentEvent(paymentEvent)
            insertPaymentOrders(paymentEvent, paymentEventId)
        }
    }

    private suspend fun insertPaymentEvent(paymentEvent: PaymentEvent): Long {
        databaseClient.sql(INSERT_PAYMENT_EVENT_QUERY)
            ...
            .rowsUpdated()
            .awaitSingle()
        return selectPaymentEventId()
    }

    private suspend fun selectPaymentEventId() = databaseClient.sql(LAST_INSERT_ID_QUERY)
        .fetch()
        .first()
        .map { (it["LAST_INSERT_ID()"] as BigInteger).toLong() }
        .awaitSingle()

    private suspend fun insertPaymentOrders(
